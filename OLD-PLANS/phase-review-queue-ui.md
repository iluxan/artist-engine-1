...
- ✅ Created "Review Events" page accessible from nav
- ✅ Shows unverified events with verification status badges
- ✅ Displays all 4 verification check results (HTTP, Content, Date, Registration)
- ✅ Approve button (moves to events table with 7-day expiry)
- ✅ Reject button (deletes event permanently)
- ✅ Badge in navigation showing count of pending events
- ✅ Shows verification errors for each event
- ✅ Added regression test UI (`🧪 Run Test` button)
- ✅ Fixed route ordering bug (`/api/events/unverified` now works correctly)

**Part 5: Regression Testing**
- ✅ API endpoint: `POST /api/test/extract-regression/:id`
- ✅ UI button to trigger test from person detail page
- ✅ Test results page with quality score and verification breakdown
- ✅ Sample events display with verification badges

### ⏸️ Deferred (Not Current Priority)

**Auto-Expiry Scheduler**
- Need `scheduler.js` with cron job to run daily
- Should delete events where `expires_at < now()`
- **Status**: Infrastructure ready, not needed until we have real events

**UI Polish**
- Expiry countdown badges on approved events
- Better event cards with all details
- Group by person or date
- **Status**: Deferred until content discovery works
